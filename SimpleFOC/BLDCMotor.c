...
/******************************************************************************/
int alignSensor(SoftI2C_HandleTypeDef *IICWHERE);
...
//	GPIO_ResetBits(GPIOC,GPIO_Pin_13);                 //上电点亮LED
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10;          //PB9是motor1的使能
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_10MHz;
	GPIO_Init(GPIOB, &GPIO_InitStructure);
	GPIO_ResetBits(GPIOB,GPIO_Pin_10);                  //低电平解除,Motor_init()中使能
}
...
/******************************************************************************/
void Motor_initFOC(SoftI2C_HandleTypeDef *IICWHERE)
{
	alignSensor(IICWHERE);    //检测零点偏移量和极对数
	//added the shaft_angle update
	angle_prev=getAngle_soft(IICWHERE);  //getVelocity(),make sure velocity=0 after power on
	delay_ms(5);
	shaft_velocity = shaftVelocity(IICWHERE);  //必须调用一次，进入主循环后速度为0
	delay_ms(5);
	shaft_angle = shaftAngle(IICWHERE);// shaft angle
...
int alignSensor(SoftI2C_HandleTypeDef *IICWHERE)
{
	...
	mid_angle=getAngle_soft(IICWHERE);
	...
	end_angle=getAngle_soft(IICWHERE);
	...
	delay_ms(700);
	zero_electric_angle = _normalizeAngle(_electricalAngle(sensor_direction*getAngle_soft(IICWHERE), pole_pairs));
	delay_ms(20);
...
/******************************************************************************/
void loopFOC(SoftI2C_HandleTypeDef *IICWHERE)
{
...
/******************************************************************************/
void move(SoftI2C_HandleTypeDef *IICWHERE,float new_target)
{
	shaft_velocity = shaftVelocity(IICWHERE);
...
void setPhaseVoltage(float Uq, float Ud, float angle_el)
{
	...
	TIM_SetCompare2(TIM2,Ta*PWM_Period);
	TIM_SetCompare3(TIM2,Tb*PWM_Period);
	TIM_SetCompare4(TIM2,Tc*PWM_Period);
}
/******************************************************************************/
float velocityOpenloop(float target_velocity)
{
